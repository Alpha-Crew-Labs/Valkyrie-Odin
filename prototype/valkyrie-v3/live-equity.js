...
(function(){
  ...
  function fundamentalCardHtml(){var d=LIVE.data||{},r=d.research,s=d.scanner;if(!r||!r.ok)return '<div class="cd fundq degraded" id="cd_fundamental_live"><div class="cdh"><span class="cdt">EQUITY FUNDAMENTAL PULSE</span><span class="cdo">DIRECT API PENDING</span></div><div class="note">실제 DART/FSC Research feed를 기다리는 중입니다. <b>샘플 기업 점수는 표시하지 않습니다.</b></div>'+scannerStats(s)+'</div>';var e=r.earnings||{},hl=r.highsLows52w||{},mb=r.marketBreadth||{},growth=Array.isArray(r.growthTop)?r.growthTop:[],fresh=(r.stale?'STALE · ':'AS-OF · ')+shortDate(r.quoteAsOf);return '<div class="cd fundq'+(r.stale?' stale':'')+'" id="cd_fundamental_live"><div class="cdh"><span class="cdt">EQUITY FUNDAMENTAL PULSE</span><span class="cdo">'+esc(fresh)+'</span></div><div class="fundq-signal"><span>DART EARNINGS BREADTH</span><strong>'+esc(e.improveRatePct==null?'—':num(e.improveRatePct,0)+'%')+'</strong><em>'+esc(e.reportedN==null?'—':Number(e.reportedN).toLocaleString('ko-KR')+'개')+'</em></div><div class="fundq-grid"><div><span>흑자전환</span><b>'+esc(e.turnaroundCount==null?'—':Number(e.turnaroundCount).toLocaleString('ko-KR')+'개')+'</b></div><div><span>52주 신고가 / 신저가</span><b>'+esc(hl&&hl.n_high==null?'—':hl.n_high)+' / '+esc(hl&&hl.n_low==null?'—':hl.n_low)+'</b></div><div><span>시장 상승비중</span><b>'+esc(mb&&mb.advance_ratio_ex_flat_pct==null?'—':num(mb.advance_ratio_ex_flat_pct,1)+'%')+'</b></div><div><span>Research 기준</span><b>'+esc(shortDate(r.quoteAsOf))+'</b></div></div>'+disclosureLine(r)+scannerStats(s)+'<div class="fundq-head">실적 성장 상위 · DART/FSC 실제 수치</div><div class="fundq-list">'+growth.map(growthRow).join('')+'</div><div class="note fundq-note">'+(r.stale?'<b>Research 기준일이 오래되어 Live 점수에는 반영하지 않습니다.</b> ':'')+'시장 시세는 Naver 공개 JSON, Fundamental은 CORS가 열린 aikstockdata/DART 계층을 브라우저에서 직접 조회합니다.</div><div class="liveq-source"><i></i><span>DIRECT API · AIKSTOCKDATA · DART/FSC</span><em>'+esc(shortDate(r.quoteAsOf))+'</em></div></div>';}

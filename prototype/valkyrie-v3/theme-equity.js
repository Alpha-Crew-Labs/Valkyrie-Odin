 ...
 * Theme ranking -> selected theme constituents. Direct first; same-origin snapshot fallback.
 */
(function(){
  'use strict';
  ...
  function themeRows(raw){return findArray(raw,function(x){return x&&x.no!=null&&x.name!=null&&x.changeRate!=null;});}
  function stockRows(raw){return findArray(raw,function(x){return x&&x.itemcode!=null&&x.itemname!=null;});}
  function normLead(x){
    var a=[];var src=Array.isArray(x&&x.leadingItem)?x.leadingItem:[];
    for(var i=0;i<src.length;i++){var r=src[i]||{};a.push({code:String(r.itemcode||r.itemCode||r.code||''),name:r.itemname||r.itemName||r.name||''});}
  ...
  function rank(themes){
    ...
    var top=a.filter(function(x){return x.changeRate>0;}).slice(0,5);
    var bottom=a.filter(function(x){return x.changeRate<0;}).slice().sort(function(a,b){return a.changeRate-b.changeRate;}).slice(0,5);
    if(!top.length)top=a.slice(0,5);
    if(!bottom.length)bottom=a.slice(-5).reverse();
    return {top:top,bottom:bottom};
  }
  async function directThemes(){
    var all=[],seen={},pageSize=100;
    for(var start=0;start<500;start+=pageSize){
      var raw=await fetchJson(BASE+'/list?startIdx='+start+'&pageSize='+pageSize+'&sortType=changeRate');
      var rows=themeRows(raw),added=0;
      for(var i=0;i<rows.length;i++){var id=String(rows[i].no);if(!seen[id]){seen[id]=1;all.push(rows[i]);added++;}}
      if(rows.length<pageSize||added===0)break;
    }
    var themes=uniqThemes(all);if(themes.length<10)throw new Error('insufficient theme rows: '+themes.length);return themes;
  }
  async function snapshot(){var p=await fetchJson(SNAPSHOT+'?t='+Date.now(),5000);if(!p||p.ok!==true||!Array.isArray(p.themes)||p.themes.length<5)throw new Error('theme snapshot unavailable');return p;}
  async function directStocks(no){
    ...
    if(!all.length)throw new Error('constituents unavailable');return all;
  }
  ...
  function themeButton(t,side){
    ...
    return '<button type="button" class="themeq-row '+tone(t.changeRate)+(STATE.selected&&STATE.selected.no===t.no?' on':'')+'" data-theme-no="'+esc(t.no)+'">'
      +'<span class="themeq-rank">'+(side==='up'?'▲':'▼')+'</span><span class="themeq-name"><b>'+esc(t.name)+'</b><small>'+esc(lead||((t.riseCnt||0)+'↑ · '+(t.fallCnt||0)+'↓'))+'</small></span>'
  ...
  function render(){
    ...
    var top=STATE.top.map(function(t){return themeButton(t,'up');}).join('');
    var bottom=STATE.bottom.map(function(t){return themeButton(t,'down');}).join('');
    ...
    root.innerHTML='<div class="themeq-head"><div><span>THEME FLOW</span><b>NAVER PAY SECURITIES · PUBLIC</b></div><div class="themeq-meta"><i class="'+(STATE.mode==='DIRECT_API'?'live':'')+'"></i>'+esc(STATE.mode==='DIRECT_API'?'DIRECT':'SNAPSHOT')+' · '+esc(stamp(STATE.generatedAt))+'</div></div>'
      ...
      +'<div class="themeq-foot">테마 수익률·상승/하락 종목수·구성종목은 Naver/Npay 공개 read-only API 기준 · 개인 API/Vercel/가짜 fallback 없음</div>';
